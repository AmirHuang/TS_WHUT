...
class ImageSerializer(serializers.ModelSerializer):
    ...
    def get_size(self, obj):
        # 图片大小
        print('obj.image.size', obj.image.size)
        return obj.image.size

    def get_image(self, obj):
        print('----------')
        print(self.context['request']._request.META['HTTP_HOST'])
        print(obj.image['avatar'].url)
        # 图片
    ...
    def get_if_like(self, obj):
        ...
        if not self.context['request'].user.is_authenticated:
            return False
        ship = LikeShip.objects.filter(user=self.context['request'].user, image=obj)
        if ship.count():
            return ship[0].id
        return False
    ...
    def get_if_collect(self, obj):
        ...
        if not self.context['request'].user.is_authenticated:
            return False
        ship = UserFolderImage.objects.filter(user=self.context['request'].user, image=obj)
        if ship.count():
            return ship[0].id
        return False
...
class CommentListSerializer(serializers.ModelSerializer):
    ...
    def get_if_like(self, obj):
        ...
        ship = CommentLike.objects.filter(user=user, comment=obj)
        if ship.count():
            return ship[0].id
        return False
...
class CommentCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not attrs['is_add_info']:
            ...
            reply = attrs['reply']
            UserMessage(post_user=self.context['request'].user.username,
                        user=reply,
                        message=attrs['content']).save()
        return attrs
