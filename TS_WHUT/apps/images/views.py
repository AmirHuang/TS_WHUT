...
class GroupsViewset(mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        images = ImageModel.objects.filter(groupimage__in=small_groups, if_active=True).order_by('-add_time')
...
class SmallGroupsViewset(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    pagination_class = ImageSerializer

    def retrieve(self, request, *args, **kwargs):
        instance = self.get_object()

        groups = GroupImage.objects.filter(group=instance)
        images = ImageModel.objects.filter(groupimage__in=groups,
                                           if_active=True).order_by('-add_time')
        page = self.paginate_queryset(images)
        if page is not None:
            serializer = self.get_serializer(page, many=True)
            return self.get_paginated_response(serializer.data)
...
class CommentViewset(mixins.ListModelMixin, mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def get_reply(self, image, start, end):
        queryset = Comment.objects.filter(is_add_info=True,
                                          image_id=image,
                                          floor__gte=start,
                                          floor__lte=end).order_by("floor")
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            if serializer.data:
                ...
                print('end--', end)
                replys = self.get_reply(int(request.GET.get('image')),
                                        start, end)
                data = serializer.data
                replys_len = len(replys)
                print('replys_len--', replys_len)
                print('data--', replys)
                ...
                for one in data:
                    ...
                    while i < replys_len and one['floor'] >= replys[i]['floor']:
                        if one['floor'] == replys[i]['floor']:
                            print('one的类型--', type(one))
                            # one的类型-- <class 'collections.OrderedDict'>
