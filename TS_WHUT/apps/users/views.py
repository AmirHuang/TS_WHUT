...
from .serializer import (FolderCreateSerializer, FolderListSerializer, FolderOneSerializer, FolderUpdateSerializer,
                         UserListSerializer, UserCreateSerializer, UserUpdateSerializer, OrgSerializer,
                         OrgOneSerializer)
from .models import Folder, UserMessage, Org, EmailVerifyRecord
...
# class ResetEmailViewset(mixins.CreateModelMixin, viewsets.GenericViewSet):
...
#
#         sms_status = send_xx_email(email)
#
#         print('-------------', sms_status)
#         pass
#
...
class ResetEmail(View):
    def post(self, request):
        ...
        user_message.save()
        send_xx_email(email, "update_email")
        pass
...
class OrgViewset(mixins.CreateModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    #     if serializer.data:
    #         print(serializer.data[0]['status'])
    #         print(serializer.data[0]['status'])
...
class UserViewset(viewsets.ModelViewSet):
    ...
    """
    throttle_classes = (UserRateThrottle, AnonRateThrottle)
    serializer_class = UserRateThrottle
    pagination_class = UserPagination
    filter_backends = (DjangoFilterBackend, filters.OrderingFilter)
    filter_fields = ('if_sign', )
            ...
            if all_records:
                for record in all_records:
                    ...
                    if not user.is_active:
                        ...
                        return User.objects.filter(email=record.send_email)
    ...
    def create(self, request, *args, **kwargs):
        ...
        # 发送验证邮箱
        send_xx_email(user.email, "register")
    ...
    @list_route(methods=['GET'], url_path='active')
    def active_uesr(self, request, *args, **kwargs):
